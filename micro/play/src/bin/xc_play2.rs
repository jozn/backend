...
use cdrs::types::{ByName, IntoRustByName};
...
impl User {
    ...
    // pub fn save<LB>(&mut self, session: &Session<LB>) -> Result<(),CWError> {
    // pub fn save<T,U>(&mut self, session: impl QueryExecutor<T,U>) -> Result<(),CWError> {
    ...
    pub fn delete<LB>(&mut self, session: &Session<LB>) -> Result<(), CWError> {
...
impl User_Selector {
    ...
    pub fn _to_cql(&self) -> (String, Vec<Value>) {
        ...
        let mut cql_query = format!("SELECT {} FROM msgs.user", cql_select);
...
impl User_Updater {
    ...
    pub fn update<LB>(&mut self, session: &Session<LB>) -> cdrs::error::Result<Frame> {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE msgs.user SET {}", cql_update)
        } else {
            format!("UPDATE msgs.user SET {} WHERE {}", cql_update, cql_where)
        };
...
impl User_Deleter {
    ...
    //each column delete
    pub fn delete_user_cid(&mut self) -> &mut Self {
    ...
    pub fn delete<LB>(&mut self, session: &Session<LB>) -> Result<(), CWError> {
        ...
        let cql_query = format!("DELETE {} FROM msgs.user WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM msgs.user WHERE " + where_str ;
