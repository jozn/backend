...
impl Messages12 {
    ...
    pub fn save(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO msgs.messages12 ({}) VALUES ({})", cql_columns, cql_question);
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        let mut deleter = Messages12_Deleter::new();
...
pub struct Messages12_Selector {
...
impl Messages12_Selector {
    pub fn new() -> Self {
        Messages12_Selector::default()
    }
    ...
    pub fn _to_cql(&self) ->  (String, Vec<Value>)  {
        ...
        let mut cql_query = format!("SELECT {} FROM msgs.messages12", cql_select);
    ...
    pub fn _get_rows_with_size(&mut self,session: impl FCQueryExecutor, size: i64) -> Result<Vec<Messages12>, CWError>   {
        ...
        for db_row in db_raws {
            let mut row = Messages12::default();
            
                
            row.chat_id = db_row.by_name("chat_id")?.unwrap_or_default();
                
            row.data = db_row.by_name::<Blob>("data")?.unwrap_or(Blob::new(vec![])).into_vec();
    ...
    pub fn get_rows(&mut self, session: impl FCQueryExecutor) -> Result<Vec<Messages12>, CWError>{
        self._get_rows_with_size(session,-1)
    }

    pub fn get_row(&mut self, session: impl FCQueryExecutor) -> Result<Messages12, CWError>{
...
#[derive(Default, Debug)]
pub struct Messages12_Deleter {
    wheres: Vec<WhereClause>,
    delete_cols: Vec<&'static str>,
}

#[derive(Default, Debug)]
pub struct Messages12_Updater {
    wheres: Vec<WhereClause>,
    updates: HashMap<&'static str, Value>,
}

impl Messages12_Updater {
    pub fn new() -> Self {
        Messages12_Updater::default()
    }
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE msgs.messages12 SET {}", cql_update)
        } else {
            format!("UPDATE msgs.messages12 SET {} WHERE {}", cql_update, cql_where)
        };
    ...
    pub fn update_data(&mut self, val: &Vec<u8>) -> &mut Self {
        self.updates.insert("data = ?", Blob::new(val.clone()).into());
        self
    }
...
impl Messages12_Deleter {
    pub fn new() -> Self {
        Messages12_Deleter::default()
    }
    ...
    pub fn delete_data(&mut self) -> &mut Self {
        self.delete_cols.push("data");
        self
    }
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("DELETE {} FROM msgs.messages12 WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM msgs.messages12 WHERE " + where_str ;
