...
impl File {
    ...
    pub fn save(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO msgs.file ({}) VALUES ({})", cql_columns, cql_question);
...
impl File_Selector {
    ...
    pub fn _to_cql(&self) ->  (String, Vec<Value>)  {
        ...
        let mut cql_query = format!("SELECT {} FROM msgs.file", cql_select);
...
impl File_Updater {
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE msgs.file SET {}", cql_update)
        } else {
            format!("UPDATE msgs.file SET {} WHERE {}", cql_update, cql_where)
        };
...
impl File_Deleter {
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("DELETE {} FROM msgs.file WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM msgs.file WHERE " + where_str ;
