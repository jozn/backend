...
impl Messages {
    ...
    pub fn save(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO msgs.messages ({}) VALUES ({})", cql_columns, cql_question);
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        let mut deleter = Messages_Deleter::new();
...
impl Messages_Selector {
    pub fn new() -> Self {
        Messages_Selector::default()
    }
    ...
    pub fn _to_cql(&self) ->  (String, Vec<Value>)  {
        ...
        let mut cql_query = format!("SELECT {} FROM msgs.messages", cql_select);
    ...
    pub fn _get_rows_with_size(&mut self,session: impl FCQueryExecutor, size: i64) -> Result<Vec<Messages>, CWError>   {
    ...
    pub fn get_rows(&mut self, session: impl FCQueryExecutor) -> Result<Vec<Messages>, CWError>{
        self._get_rows_with_size(session,-1)
    }

    pub fn get_row(&mut self, session: impl FCQueryExecutor) -> Result<Messages, CWError>{
...
#[derive(Default, Debug)]
pub struct Messages_Deleter {
    wheres: Vec<WhereClause>,
    delete_cols: Vec<&'static str>,
}

#[derive(Default, Debug)]
pub struct Messages_Updater {
    wheres: Vec<WhereClause>,
    updates: HashMap<&'static str, Value>,
}

impl Messages_Updater {
    pub fn new() -> Self {
        Messages_Updater::default()
    }
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE msgs.messages SET {}", cql_update)
        } else {
            format!("UPDATE msgs.messages SET {} WHERE {}", cql_update, cql_where)
        };
...
impl Messages_Deleter {
    pub fn new() -> Self {
        Messages_Deleter::default()
    }
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("DELETE {} FROM msgs.messages WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM msgs.messages WHERE " + where_str ;
