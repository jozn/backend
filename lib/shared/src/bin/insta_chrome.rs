...
async fn insta() {
    ...
    // Accept Cookies
    let els = driver.find_elements(By::Tag("button")).await.unwrap();
    for c in els {
    ...
    // Submit Form (2 buttons: 1) show password  2) log in
    ...
    let m = driver
        .get("https://www.instagram.com/pugloulou/?__a=1")
        .await
        .unwrap();
...
async fn wiki() {
    ...
    driver
        .find_element(By::ClassName("firstHeading"))
        .await
        .unwrap();
