...
impl ChannelMsg {
    pub async fn insert(&self, spool: &SPool) -> Result<ChannelMsg,MyError> {
        ...
        let query = format!(r"INSERT INTO {:}.channel_msg (channel_id, msg_gid, pb_data, debug_data) VALUES (?, ?, ?, ?)",&spool.database);
        let p = Params::Positional(vec![self.channel_id.clone().into(), self.msg_gid.clone().into(), self.pb_data.clone().into(), self.debug_data.clone().into()]);
    ...
    pub async fn replace(&self, spool: &SPool) -> Result<ChannelMsg,MyError> {
        ...
        let query = format!(r"REPLACE INTO {:}.channel_msg (channel_id, msg_gid, pb_data, debug_data) VALUES (?, ?, ?, ?)",&spool.database);
        let p = Params::Positional(vec![self.channel_id.clone().into(), self.msg_gid.clone().into(), self.pb_data.clone().into(), self.debug_data.clone().into()]);
    ...
    pub async fn update(&self, spool: &SPool) -> Result<(),MyError> {
        let mut conn = spool.pool.get_conn().await?;
        let query = format!(r"UPDATE `{:}`.channel_msg` SET pb_data = ?, debug_data = ? WHERE channel_id = ? AND msg_gid = ? ", &spool.database);
        let p = Params::Positional(vec![self.pb_data.clone().into(), self.debug_data.clone().into(),  self.channel_id.clone().into(), self.msg_gid.clone().into() ]);
    ...
    pub async fn delete(&self, spool: &SPool) -> Result<(),MyError> {
        ...
        let query = format!(r"DELETE FROM `{:}`.channel_msg` WHERE channel_id = ? AND msg_gid = ? ", &spool.database);
        let p = Params::Positional(vec![self.channel_id.clone().into(), self.msg_gid.clone().into()]);
...
pub async fn channel_msg_mass_insert(arr :&Vec<ChannelMsg>, spool: &SPool) -> Result<(),MyError> {
    ...
    if arr.len() == 0 {
     ...
     let mut insert_fields = "(channel_id, msg_gid, pb_data, debug_data)";
...
// Index
pub async fn get_channel_msg(channel_id: u32,msg_gid: u64, spool: &SPool) -> Result<ChannelMsg,MyError> {
