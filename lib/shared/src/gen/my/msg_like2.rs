...
impl MsgLike2 {
    pub async fn insert(&self, spool: &SPool) -> Result<MsgLike2,MyError> {
        let mut conn = spool.pool.get_conn().await?;

        let query = format!(r"INSERT INTO {:}.msg_like2 (message_gid, profile_cid) VALUES (?, ?)",&spool.database);
    ...
    pub async fn replace(&self, spool: &SPool) -> Result<MsgLike2,MyError> {
        let mut conn = spool.pool.get_conn().await?;

        let query = format!(r"REPLACE INTO {:}.msg_like2 (message_gid, profile_cid) VALUES (?, ?)",&spool.database);
        let p = Params::Positional(vec![self.message_gid.clone().into(), self.profile_cid.clone().into()]);
    ...
    pub async fn update(&self, spool: &SPool) -> Result<(),MyError> {
        let mut conn = spool.pool.get_conn().await?;
        let query = format!(r"UPDATE `{:}`.msg_like2` SET  WHERE message_gid = ? AND profile_cid = ? ", &spool.database);
        let p = Params::Positional(vec![ self.message_gid.clone().into(), self.profile_cid.clone().into() ]);
    ...
    pub async fn delete(&self, spool: &SPool) -> Result<(),MyError> {
        ...
        let query = format!(r"DELETE FROM `{:}`.msg_like2` WHERE message_gid = ? AND profile_cid = ? ", &spool.database);
        let p = Params::Positional(vec![self.message_gid.clone().into(), self.profile_cid.clone().into()]);
...
impl MsgLike2Selector {
    ...
    pub async fn _get_rows_with_size(&mut self, session: &SPool, size: i64) -> Result<Vec<MsgLike2>, MyError>   {
    ...
    pub async fn get_rows(&mut self, session: &SPool) -> Result<Vec<MsgLike2>, MyError>{
        self._get_rows_with_size(session,-1).await
    }

    pub async fn get_row(&mut self, session: &SPool) -> Result<MsgLike2, MyError>{
...
#[derive(Default, Debug)]
pub struct MsgLike2Updater {
    q: TQuery
}

impl MsgLike2Updater {
    pub fn new() -> Self {
        MsgLike2Updater{
            q: TQuery{
                table: "msg_like2",
...
#[derive(Default, Debug)]
pub struct MsgLike2Deleter {
    q: TQuery
}

impl MsgLike2Deleter {
    pub fn new() -> Self {
        MsgLike2Deleter{
            q: TQuery{
                table: "msg_like2",
...
pub async fn msg_like2_mass_insert(arr :&Vec<MsgLike2>, spool: &SPool) -> Result<(),MyError> {
    let mut conn = spool.pool.get_conn().await?;
    if arr.len() == 0 {
...
// Index
pub async fn get_msg_like2(message_gid: u64,profile_cid: u64, spool: &SPool) -> Result<MsgLike2,MyError> {
	let m = MsgLike2Selector::new()
