...
impl Direct {
    pub async fn insert(&self, spool: &SPool) -> Result<Direct,MyError> {
        ...
        let query = format!(r"INSERT INTO {:}.direct (profile_id, direct_gid, msg_gid, pb_data, debug_data) VALUES (?, ?, ?, ?, ?)",&spool.database);
        let p = Params::Positional(vec![self.profile_id.clone().into(), self.direct_gid.clone().into(), self.msg_gid.clone().into(), self.pb_data.clone().into(), self.debug_data.clone().into()]);
    ...
    pub async fn replace(&self, spool: &SPool) -> Result<Direct,MyError> {
        ...
        let query = format!(r"REPLACE INTO {:}.direct (profile_id, direct_gid, msg_gid, pb_data, debug_data) VALUES (?, ?, ?, ?, ?)",&spool.database);
        let p = Params::Positional(vec![self.profile_id.clone().into(), self.direct_gid.clone().into(), self.msg_gid.clone().into(), self.pb_data.clone().into(), self.debug_data.clone().into()]);
    ...
    pub async fn update(&self, spool: &SPool) -> Result<(),MyError> {
        let mut conn = spool.pool.get_conn().await?;
        let query = format!(r"UPDATE `{:}`.direct` SET pb_data = ?, debug_data = ? WHERE profile_id = ? AND direct_gid = ? AND msg_gid = ? ", &spool.database);
        let p = Params::Positional(vec![self.pb_data.clone().into(), self.debug_data.clone().into(),  self.profile_id.clone().into(), self.direct_gid.clone().into(), self.msg_gid.clone().into() ]);
    ...
    pub async fn delete(&self, spool: &SPool) -> Result<(),MyError> {
        ...
        let query = format!(r"DELETE FROM `{:}`.direct` WHERE profile_id = ? AND direct_gid = ? AND msg_gid = ? ", &spool.database);
        let p = Params::Positional(vec![self.profile_id.clone().into(), self.direct_gid.clone().into(), self.msg_gid.clone().into()]);
...
pub async fn direct_mass_insert(arr :&Vec<Direct>, spool: &SPool) -> Result<(),MyError> {
    ...
    if arr.len() == 0 {
     ...
     let mut insert_fields = "(profile_id, direct_gid, msg_gid, pb_data, debug_data)";
...
// Index
pub async fn get_direct(profile_id: u32,direct_gid: u64,msg_gid: u64, spool: &SPool) -> Result<Direct,MyError> {
