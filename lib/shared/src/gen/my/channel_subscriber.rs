...
impl ChannelSubscriber {
    pub async fn insert(&self, spool: &SPool) -> Result<ChannelSubscriber,MyError> {
        ...
        let query = format!(r"INSERT INTO {:}.channel_subscriber (channel_id, profile_id, subscriber_gid) VALUES (?, ?, ?)",&spool.database);
        let p = Params::Positional(vec![self.channel_id.clone().into(), self.profile_id.clone().into(), self.subscriber_gid.clone().into()]);
    ...
    pub async fn replace(&self, spool: &SPool) -> Result<ChannelSubscriber,MyError> {
        ...
        let query = format!(r"REPLACE INTO {:}.channel_subscriber (channel_id, profile_id, subscriber_gid) VALUES (?, ?, ?)",&spool.database);
        let p = Params::Positional(vec![self.channel_id.clone().into(), self.profile_id.clone().into(), self.subscriber_gid.clone().into()]);
    ...
    pub async fn update(&self, spool: &SPool) -> Result<(),MyError> {
        let mut conn = spool.pool.get_conn().await?;
        let query = format!(r"UPDATE `{:}`.channel_subscriber` SET  WHERE channel_id = ? AND profile_id = ? AND subscriber_gid = ? ", &spool.database);
        let p = Params::Positional(vec![ self.channel_id.clone().into(), self.profile_id.clone().into(), self.subscriber_gid.clone().into() ]);
    ...
    pub async fn delete(&self, spool: &SPool) -> Result<(),MyError> {
        ...
        let query = format!(r"DELETE FROM `{:}`.channel_subscriber` WHERE channel_id = ? AND profile_id = ? AND subscriber_gid = ? ", &spool.database);
        let p = Params::Positional(vec![self.channel_id.clone().into(), self.profile_id.clone().into(), self.subscriber_gid.clone().into()]);
...
pub async fn channel_subscriber_mass_insert(arr :&Vec<ChannelSubscriber>, spool: &SPool) -> Result<(),MyError> {
    ...
    if arr.len() == 0 {
     ...
     let mut insert_fields = "(channel_id, profile_id, subscriber_gid)";
...
// Index
pub async fn get_channel_subscriber(channel_id: u32,profile_id: u32,subscriber_gid: u64, spool: &SPool) -> Result<ChannelSubscriber,MyError> {
