...
impl TgChannel {
    pub async fn replace(&self, pool: &Pool) -> Result<TgChannel, MyError> {
        ...
        let query =
            r"REPLACE INTO flip_tg.tg_channel (channel_id, username, data) VALUES (?, ?, ?)";
        let p = Params::Positional(vec![
...
impl TgChannelSelector {
    ...
    pub async fn _get_rows_with_size(
        ...
        let query_result = conn.exec_map(cql_query, p, |obj: TgChannel| obj).await?;
...
impl TgChannelMsg {
    pub async fn replace(&self, pool: &Pool) -> Result<TgChannelMsg, MyError> {
        ...
        let p = Params::Positional(vec![
            ...
            self.seq_id.clone().into(),
            self.flip_gid.clone().into(),
            self.deleted.clone().into(),
            self.flip_sync.clone().into(),
            self.sync_time.clone().into(),
            self.data.clone().into(),
        ]);
...
impl TgChannelMsgSelector {
    ...
    pub async fn _get_rows_with_size(
        ...
        session: &Pool,
        size: i64,
    ) -> Result<Vec<TgChannelMsg>, MyError> {
        let mut conn = session.get_conn().await?;
        let (cql_query, query_values) = self._to_cql();
        ...
        let query_result = conn.exec_map(cql_query, p, |obj: TgChannelMsg| obj).await?;
...
impl TgLinkSelector {
    ...
    pub async fn _get_rows_with_size(
        ...
        let query_result = conn.exec_map(cql_query, p, |obj: TgLink| obj).await?;
...
impl TgUsernameSelector {
    ...
    pub async fn _get_rows_with_size(
        ...
        let query_result = conn.exec_map(cql_query, p, |obj: TgUsername| obj).await?;
