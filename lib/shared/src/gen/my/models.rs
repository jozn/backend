...
pub struct Channel  { // channel
    pub channel_cid: u64,
    pub pb_data: UNKNOWN_sqlToRust__,
}
...
impl Tweet {
    pub async fn replace(&self, pool: &Pool) -> Result<Tweet,MyError> {
        ...
        let query = r"REPLACE INTO flip_my.tweet (tweet_id) VALUES (?)";
        let p = Params::Positional(vec![self.tweet_id.clone().into(), self.created_time.clone().into(), self.text_body.clone().into()]);
