...
impl SmsBk {
    pub async fn insert(&self, spool: &SPool) -> Result<SmsBk,MyError> {
        let mut conn = spool.pool.get_conn().await?;

        let query = format!(r"INSERT INTO {:}.sms_bk (sms_id, phone_number, hash_code, result_code, pb_data, debug_data) VALUES (?, ?, ?, ?, ?, ?)",&spool.database);
    ...
    pub async fn update(&self, spool: &SPool) -> Result<(),MyError> {
        let mut conn = spool.pool.get_conn().await?;
        let query = format!(r"UPDATE `{:}`.sms_bk` SET sms_id = ?, phone_number = ?, hash_code = ?, result_code = ?, pb_data = ?, debug_data = ? WHERE  ", &spool.database);
        let p = Params::Positional(vec![self.sms_id.clone().into(), self.phone_number.clone().into(), self.hash_code.clone().into(), self.result_code.clone().into(), self.pb_data.clone().into(), self.debug_data.clone().into(),   ]);
    ...
    pub async fn delete(&self, spool: &SPool) -> Result<(),MyError> {
        ...
        let query = format!(r"DELETE FROM `{:}`.sms_bk` WHERE  ", &spool.database);
...
impl SmsBkSelector {
    ...
    pub async fn _get_rows_with_size(&mut self, session: &SPool, size: i64) -> Result<Vec<SmsBk>, MyError>   {
    ...
    pub async fn get_rows(&mut self, session: &SPool) -> Result<Vec<SmsBk>, MyError>{
        self._get_rows_with_size(session,-1).await
    }

    pub async fn get_row(&mut self, session: &SPool) -> Result<SmsBk, MyError>{
...
pub async fn sms_bk_mass_insert(arr :&Vec<SmsBk>, spool: &SPool) -> Result<(),MyError> {
    let mut conn = spool.pool.get_conn().await?;
    if arr.len() == 0 {
        return Err(MyError::EmptySql)
    }
     let mut insert_fields = "(sms_id, phone_number, hash_code, result_code, pb_data, debug_data)";
    ...
    for ar in arr {
        arr_vals.push(ar.sms_id.clone().into());
