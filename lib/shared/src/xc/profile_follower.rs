...
impl ProfileFollower {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO flip.profile_follower ({}) VALUES ({})", cql_columns, cql_question);
    ...
    pub fn delete(&self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        let mut deleter = ProfileFollower_Deleter::new();
...
pub struct ProfileFollower_Selector {
...
impl ProfileFollower_Selector {
    pub fn new() -> Self {
        ProfileFollower_Selector::default()
    }
    ...
    pub fn _to_cql(&self) ->  (String, Vec<Value>)  {
        ...
        let mut cql_query = format!("SELECT {} FROM flip.profile_follower", cql_select);
    ...
    pub fn _get_rows_with_size(&mut self,session: impl FCQueryExecutor, size: i64) -> Result<Vec<ProfileFollower>, CWError>   {
        ...
        for db_row in db_raws {
            let mut row = ProfileFollower::default();
    ...
    pub fn get_rows(&mut self, session: impl FCQueryExecutor) -> Result<Vec<ProfileFollower>, CWError>{
        self._get_rows_with_size(session,-1)
    }

    pub fn get_row(&mut self, session: impl FCQueryExecutor) -> Result<ProfileFollower, CWError>{
...
#[derive(Default, Debug)]
pub struct ProfileFollower_Deleter {
    wheres: Vec<WhereClause>,
    delete_cols: Vec<&'static str>,
}

#[derive(Default, Debug)]
pub struct ProfileFollower_Updater {
    wheres: Vec<WhereClause>,
    updates: HashMap<&'static str, Value>,
}

impl ProfileFollower_Updater {
    pub fn new() -> Self {
        ProfileFollower_Updater::default()
    }
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE flip.profile_follower SET {}", cql_update)
        } else {
            format!("UPDATE flip.profile_follower SET {} WHERE {}", cql_update, cql_where)
        };
...
impl ProfileFollower_Deleter {
    pub fn new() -> Self {
        ProfileFollower_Deleter::default()
    }
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("DELETE {} FROM flip.profile_follower WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM flip.profile_follower WHERE " + where_str ;
...
pub fn get_profile_follower_by_profile_cid_and_follow_gid_and_channel_cid(session: impl FCQueryExecutor, p1:i64,p2:i64,p3:i64) -> Result<ProfileFollower,CWError> {
	let m = ProfileFollower_Selector::new()
		.profile_cid_eq(p1)
