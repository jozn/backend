...
pub struct ProfileFollowerSort {
...
impl ProfileFollowerSort {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO flip.profile_follower_sort ({}) VALUES ({})", cql_columns, cql_question);
    ...
    pub fn delete(&self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        let mut deleter = ProfileFollowerSort_Deleter::new();
...
pub struct ProfileFollowerSort_Selector {
...
impl ProfileFollowerSort_Selector {
    pub fn new() -> Self {
        ProfileFollowerSort_Selector::default()
    }
    ...
    pub fn _to_cql(&self) ->  (String, Vec<Value>)  {
        ...
        let mut cql_query = format!("SELECT {} FROM flip.profile_follower_sort", cql_select);
    ...
    pub fn _get_rows_with_size(&mut self,session: impl FCQueryExecutor, size: i64) -> Result<Vec<ProfileFollowerSort>, CWError>   {
        ...
        for db_row in db_raws {
            let mut row = ProfileFollowerSort::default();
    ...
    pub fn get_rows(&mut self, session: impl FCQueryExecutor) -> Result<Vec<ProfileFollowerSort>, CWError>{
        self._get_rows_with_size(session,-1)
    }

    pub fn get_row(&mut self, session: impl FCQueryExecutor) -> Result<ProfileFollowerSort, CWError>{
...
#[derive(Default, Debug)]
pub struct ProfileFollowerSort_Deleter {
    wheres: Vec<WhereClause>,
    delete_cols: Vec<&'static str>,
}

#[derive(Default, Debug)]
pub struct ProfileFollowerSort_Updater {
    wheres: Vec<WhereClause>,
    updates: HashMap<&'static str, Value>,
}

impl ProfileFollowerSort_Updater {
    pub fn new() -> Self {
        ProfileFollowerSort_Updater::default()
    }
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE flip.profile_follower_sort SET {}", cql_update)
        } else {
            format!("UPDATE flip.profile_follower_sort SET {} WHERE {}", cql_update, cql_where)
        };
...
impl ProfileFollowerSort_Deleter {
    pub fn new() -> Self {
        ProfileFollowerSort_Deleter::default()
    }
    ...
    pub fn delete_follow_gid(&mut self) -> &mut Self {
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("DELETE {} FROM flip.profile_follower_sort WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM flip.profile_follower_sort WHERE " + where_str ;
...
pub fn get_profile_follower_sort_by_profile_cid_and_channel_cid_and_follow_gid(session: impl FCQueryExecutor, p1:i64,p2:i64,p3:i64) -> Result<ProfileFollowerSort,CWError> {
	let m = ProfileFollowerSort_Selector::new()
		.profile_cid_eq(p1)
		.and_channel_cid_eq(p2)
		.and_follow_gid_eq(p3)
		.get_row(session)?;
	Ok(m)
}
