...
impl ChannelSubscriber {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO flip.channel_subscriber ({}) VALUES ({})", cql_columns, cql_question);
    ...
    pub fn delete(&self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        let mut deleter = ChannelSubscriber_Deleter::new();
...
pub struct ChannelSubscriber_Selector {
...
impl ChannelSubscriber_Selector {
    pub fn new() -> Self {
        ChannelSubscriber_Selector::default()
    }
    ...
    pub fn _to_cql(&self) ->  (String, Vec<Value>)  {
        ...
        let mut cql_query = format!("SELECT {} FROM flip.channel_subscriber", cql_select);
    ...
    pub fn _get_rows_with_size(&mut self,session: impl FCQueryExecutor, size: i64) -> Result<Vec<ChannelSubscriber>, CWError>   {
        ...
        for db_row in db_raws {
            let mut row = ChannelSubscriber::default();
    ...
    pub fn get_rows(&mut self, session: impl FCQueryExecutor) -> Result<Vec<ChannelSubscriber>, CWError>{
        self._get_rows_with_size(session,-1)
    }

    pub fn get_row(&mut self, session: impl FCQueryExecutor) -> Result<ChannelSubscriber, CWError>{
...
#[derive(Default, Debug)]
pub struct ChannelSubscriber_Deleter {
    wheres: Vec<WhereClause>,
    delete_cols: Vec<&'static str>,
}

#[derive(Default, Debug)]
pub struct ChannelSubscriber_Updater {
    wheres: Vec<WhereClause>,
    updates: HashMap<&'static str, Value>,
}

impl ChannelSubscriber_Updater {
    pub fn new() -> Self {
        ChannelSubscriber_Updater::default()
    }
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE flip.channel_subscriber SET {}", cql_update)
        } else {
            format!("UPDATE flip.channel_subscriber SET {} WHERE {}", cql_update, cql_where)
        };
...
impl ChannelSubscriber_Deleter {
    pub fn new() -> Self {
        ChannelSubscriber_Deleter::default()
    }
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("DELETE {} FROM flip.channel_subscriber WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM flip.channel_subscriber WHERE " + where_str ;
...
pub fn get_channel_subscriber_by_channel_cid_and_subscriber_gid_and_profile_cid(session: impl FCQueryExecutor, p1:i64,p2:i64,p3:i64) -> Result<ChannelSubscriber,CWError> {
	let m = ChannelSubscriber_Selector::new()
		.channel_cid_eq(p1)
