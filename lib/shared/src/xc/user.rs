...
impl User {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO flip.user ({}) VALUES ({})", cql_columns, cql_question);
...
impl User_Selector {
    ...
    pub fn _to_cql(&self) ->  (String, Vec<Value>)  {
        ...
        let mut cql_query = format!("SELECT {} FROM flip.user", cql_select);
...
impl User_Updater {
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE flip.user SET {}", cql_update)
        } else {
            format!("UPDATE flip.user SET {} WHERE {}", cql_update, cql_where)
        };
...
impl User_Deleter {
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("DELETE {} FROM flip.user WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM flip.user WHERE " + where_str ;
