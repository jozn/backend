...
impl MsgReshare {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        ...
        let cql_query = format!(
            "INSERT INTO msgs.msg_reshare ({}) VALUES ({})",
...
impl MsgReshare_Selector {
    ...
    pub fn _to_cql(&self) -> (String, Vec<Value>) {
        ...
        let mut cql_query = format!("SELECT {} FROM msgs.msg_reshare", cql_select);
...
impl MsgReshare_Updater {
    ...
    pub fn update(&mut self, session: impl FCQueryExecutor) -> cdrs::error::Result<Frame> {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE msgs.msg_reshare SET {}", cql_update)
        } else {
            format!(
                "UPDATE msgs.msg_reshare SET {} WHERE {}",
...
impl MsgReshare_Deleter {
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        ...
        let cql_query = format!(
            "DELETE {} FROM msgs.msg_reshare WHERE {}",
            del_col, where_str
        );
        //let cql_query = "DELETE " + del_col + " FROM msgs.msg_reshare WHERE " + where_str ;
...
pub fn get_msg_reshare_by_message_gid_and_profile_cid(
    session: impl FCQueryExecutor,
    p1: i64,
    p2: i64,
) -> Result<MsgReshare, CWError> {
