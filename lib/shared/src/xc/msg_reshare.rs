...
impl MsgReshare {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO flip.msg_reshare ({}) VALUES ({})", cql_columns, cql_question);
    ...
    pub fn delete(&self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        let mut deleter = MsgReshare_Deleter::new();
...
pub struct MsgReshare_Selector {
...
impl MsgReshare_Selector {
    pub fn new() -> Self {
        MsgReshare_Selector::default()
    }
    ...
    pub fn _to_cql(&self) ->  (String, Vec<Value>)  {
        ...
        let mut cql_query = format!("SELECT {} FROM flip.msg_reshare", cql_select);
    ...
    pub fn _get_rows_with_size(&mut self,session: impl FCQueryExecutor, size: i64) -> Result<Vec<MsgReshare>, CWError>   {
        ...
        for db_row in db_raws {
            let mut row = MsgReshare::default();
    ...
    pub fn get_rows(&mut self, session: impl FCQueryExecutor) -> Result<Vec<MsgReshare>, CWError>{
        self._get_rows_with_size(session,-1)
    }

    pub fn get_row(&mut self, session: impl FCQueryExecutor) -> Result<MsgReshare, CWError>{
...
#[derive(Default, Debug)]
pub struct MsgReshare_Deleter {
    wheres: Vec<WhereClause>,
    delete_cols: Vec<&'static str>,
}

#[derive(Default, Debug)]
pub struct MsgReshare_Updater {
    wheres: Vec<WhereClause>,
    updates: HashMap<&'static str, Value>,
}

impl MsgReshare_Updater {
    pub fn new() -> Self {
        MsgReshare_Updater::default()
    }
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE flip.msg_reshare SET {}", cql_update)
        } else {
            format!("UPDATE flip.msg_reshare SET {} WHERE {}", cql_update, cql_where)
        };
...
impl MsgReshare_Deleter {
    pub fn new() -> Self {
        MsgReshare_Deleter::default()
    }
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("DELETE {} FROM flip.msg_reshare WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM flip.msg_reshare WHERE " + where_str ;
...
pub fn get_msg_reshare_by_message_gid_and_profile_cid(session: impl FCQueryExecutor, p1:i64,p2:i64) -> Result<MsgReshare,CWError> {
	let m = MsgReshare_Selector::new()
		.message_gid_eq(p1)
