...
impl MsgLike {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO flip.msg_like ({}) VALUES ({})", cql_columns, cql_question);
...
impl MsgLike_Selector {
    ...
    pub fn _get_rows_with_size(&mut self,session: impl FCQueryExecutor, size: i64) -> Result<Vec<MsgLike>, CWError>   {
...
impl MsgLike_Updater {
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            ...
            format!("UPDATE flip.msg_like SET {} WHERE {}", cql_update, cql_where)
        };
...
pub fn get_msg_like_by_message_gid_and_profile_cid(session: impl FCQueryExecutor, p1:i64,p2:i64) -> Result<MsgLike,CWError> {
