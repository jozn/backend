...
pub struct ProfileSubscriberSort {
...
impl ProfileSubscriberSort {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO flip.profile_subscriber_sort ({}) VALUES ({})", cql_columns, cql_question);
    ...
    pub fn delete(&self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        let mut deleter = ProfileSubscriberSort_Deleter::new();
...
pub struct ProfileSubscriberSort_Selector {
...
impl ProfileSubscriberSort_Selector {
    pub fn new() -> Self {
        ProfileSubscriberSort_Selector::default()
    }
    ...
    pub fn _to_cql(&self) ->  (String, Vec<Value>)  {
        ...
        let mut cql_query = format!("SELECT {} FROM flip.profile_subscriber_sort", cql_select);
    ...
    pub fn _get_rows_with_size(&mut self,session: impl FCQueryExecutor, size: i64) -> Result<Vec<ProfileSubscriberSort>, CWError>   {
        ...
        for db_row in db_raws {
            let mut row = ProfileSubscriberSort::default();
    ...
    pub fn get_rows(&mut self, session: impl FCQueryExecutor) -> Result<Vec<ProfileSubscriberSort>, CWError>{
        self._get_rows_with_size(session,-1)
    }

    pub fn get_row(&mut self, session: impl FCQueryExecutor) -> Result<ProfileSubscriberSort, CWError>{
...
#[derive(Default, Debug)]
pub struct ProfileSubscriberSort_Deleter {
    wheres: Vec<WhereClause>,
    delete_cols: Vec<&'static str>,
}

#[derive(Default, Debug)]
pub struct ProfileSubscriberSort_Updater {
    wheres: Vec<WhereClause>,
    updates: HashMap<&'static str, Value>,
}

impl ProfileSubscriberSort_Updater {
    pub fn new() -> Self {
        ProfileSubscriberSort_Updater::default()
    }
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE flip.profile_subscriber_sort SET {}", cql_update)
        } else {
            format!("UPDATE flip.profile_subscriber_sort SET {} WHERE {}", cql_update, cql_where)
        };
...
impl ProfileSubscriberSort_Deleter {
    pub fn new() -> Self {
        ProfileSubscriberSort_Deleter::default()
    }
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("DELETE {} FROM flip.profile_subscriber_sort WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM flip.profile_subscriber_sort WHERE " + where_str ;
...
pub fn get_profile_subscriber_sort_by_profile_cid_and_channel_cid_and_subscriber_gid(session: impl FCQueryExecutor, p1:i64,p2:i64,p3:i64) -> Result<ProfileSubscriberSort,CWError> {
	let m = ProfileSubscriberSort_Selector::new()
		.profile_cid_eq(p1)
