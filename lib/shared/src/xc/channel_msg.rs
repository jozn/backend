...
impl ChannelMsg {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO flip.channel_msg ({}) VALUES ({})", cql_columns, cql_question);
    ...
    pub fn delete(&self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        let mut deleter = ChannelMsg_Deleter::new();
...
pub struct ChannelMsg_Selector {
...
impl ChannelMsg_Selector {
    pub fn new() -> Self {
        ChannelMsg_Selector::default()
    }
    ...
    pub fn _to_cql(&self) ->  (String, Vec<Value>)  {
        ...
        let mut cql_query = format!("SELECT {} FROM flip.channel_msg", cql_select);
    ...
    pub fn _get_rows_with_size(&mut self,session: impl FCQueryExecutor, size: i64) -> Result<Vec<ChannelMsg>, CWError>   {
        ...
        for db_row in db_raws {
            let mut row = ChannelMsg::default();
    ...
    pub fn get_rows(&mut self, session: impl FCQueryExecutor) -> Result<Vec<ChannelMsg>, CWError>{
        self._get_rows_with_size(session,-1)
    }

    pub fn get_row(&mut self, session: impl FCQueryExecutor) -> Result<ChannelMsg, CWError>{
...
#[derive(Default, Debug)]
pub struct ChannelMsg_Deleter {
    wheres: Vec<WhereClause>,
    delete_cols: Vec<&'static str>,
}

#[derive(Default, Debug)]
pub struct ChannelMsg_Updater {
    wheres: Vec<WhereClause>,
    updates: HashMap<&'static str, Value>,
}

impl ChannelMsg_Updater {
    pub fn new() -> Self {
        ChannelMsg_Updater::default()
    }
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE flip.channel_msg SET {}", cql_update)
        } else {
            format!("UPDATE flip.channel_msg SET {} WHERE {}", cql_update, cql_where)
        };
...
impl ChannelMsg_Deleter {
    pub fn new() -> Self {
        ChannelMsg_Deleter::default()
    }
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("DELETE {} FROM flip.channel_msg WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM flip.channel_msg WHERE " + where_str ;
...
pub fn get_channel_msg_by_channel_cid_and_msg_gid(session: impl FCQueryExecutor, p1:i64,p2:i64) -> Result<ChannelMsg,CWError> {
