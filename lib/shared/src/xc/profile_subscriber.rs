...
impl ProfileSubscriber {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO flip.profile_subscriber ({}) VALUES ({})", cql_columns, cql_question);
    ...
    pub fn delete(&self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        let mut deleter = ProfileSubscriber_Deleter::new();
...
pub struct ProfileSubscriber_Selector {
...
impl ProfileSubscriber_Selector {
    pub fn new() -> Self {
        ProfileSubscriber_Selector::default()
    }
    ...
    pub fn _to_cql(&self) ->  (String, Vec<Value>)  {
        ...
        let mut cql_query = format!("SELECT {} FROM flip.profile_subscriber", cql_select);
    ...
    pub fn _get_rows_with_size(&mut self,session: impl FCQueryExecutor, size: i64) -> Result<Vec<ProfileSubscriber>, CWError>   {
        ...
        for db_row in db_raws {
            let mut row = ProfileSubscriber::default();
    ...
    pub fn get_rows(&mut self, session: impl FCQueryExecutor) -> Result<Vec<ProfileSubscriber>, CWError>{
        self._get_rows_with_size(session,-1)
    }

    pub fn get_row(&mut self, session: impl FCQueryExecutor) -> Result<ProfileSubscriber, CWError>{
...
#[derive(Default, Debug)]
pub struct ProfileSubscriber_Deleter {
    wheres: Vec<WhereClause>,
    delete_cols: Vec<&'static str>,
}

#[derive(Default, Debug)]
pub struct ProfileSubscriber_Updater {
    wheres: Vec<WhereClause>,
    updates: HashMap<&'static str, Value>,
}

impl ProfileSubscriber_Updater {
    pub fn new() -> Self {
        ProfileSubscriber_Updater::default()
    }
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE flip.profile_subscriber SET {}", cql_update)
        } else {
            format!("UPDATE flip.profile_subscriber SET {} WHERE {}", cql_update, cql_where)
        };
...
impl ProfileSubscriber_Deleter {
    pub fn new() -> Self {
        ProfileSubscriber_Deleter::default()
    }
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("DELETE {} FROM flip.profile_subscriber WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM flip.profile_subscriber WHERE " + where_str ;
...
pub fn get_profile_subscriber_by_profile_cid_and_subscriber_gid_and_channel_cid(session: impl FCQueryExecutor, p1:i64,p2:i64,p3:i64) -> Result<ProfileSubscriber,CWError> {
	let m = ProfileSubscriber_Selector::new()
		.profile_cid_eq(p1)
		.and_subscriber_gid_eq(p2)
		.and_channel_cid_eq(p3)
		.get_row(session)?;
	Ok(m)
}
