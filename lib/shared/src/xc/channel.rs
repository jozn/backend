...
impl Channel {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("INSERT INTO flip.channel ({}) VALUES ({})", cql_columns, cql_question);
...
impl Channel_Selector {
    ...
    pub fn _to_cql(&self) ->  (String, Vec<Value>)  {
        ...
        let mut cql_query = format!("SELECT {} FROM flip.channel", cql_select);
...
impl Channel_Updater {
    ...
    pub fn update(&mut self,session: impl FCQueryExecutor) -> cdrs::error::Result<Frame>  {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE flip.channel SET {}", cql_update)
        } else {
            format!("UPDATE flip.channel SET {} WHERE {}", cql_update, cql_where)
        };
...
impl Channel_Deleter {
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(),CWError> {
        ...
        let cql_query = format!("DELETE {} FROM flip.channel WHERE {}", del_col, where_str);
        //let cql_query = "DELETE " + del_col + " FROM flip.channel WHERE " + where_str ;
...
pub fn get_channel_by_channel_cid(session: impl FCQueryExecutor, p1:i64) -> Result<Channel,CWError> {
	let m = Channel_Selector::new()
		.channel_cid_eq(p1)
		.get_row(session)?;
	Ok(m)
}
