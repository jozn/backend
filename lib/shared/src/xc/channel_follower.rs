...
impl ChannelFollower {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        ...
        let cql_query = format!(
            "INSERT INTO msgs.channel_follower ({}) VALUES ({})",
...
pub struct ChannelFollower_Selector {
...
impl ChannelFollower_Selector {
    pub fn new() -> Self {
        ChannelFollower_Selector::default()
    }
    ...
    pub fn _to_cql(&self) -> (String, Vec<Value>) {
        ...
        let mut cql_query = format!("SELECT {} FROM msgs.channel_follower", cql_select);
...
#[derive(Default, Debug)]
pub struct ChannelFollower_Updater {
    wheres: Vec<WhereClause>,
    updates: HashMap<&'static str, Value>,
}

impl ChannelFollower_Updater {
    pub fn new() -> Self {
    ...
    pub fn update(&mut self, session: impl FCQueryExecutor) -> cdrs::error::Result<Frame> {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE msgs.channel_follower SET {}", cql_update)
        } else {
            format!(
                "UPDATE msgs.channel_follower SET {} WHERE {}",
...
impl ChannelFollower_Deleter {
    pub fn new() -> Self {
        ChannelFollower_Deleter::default()
    }
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        ...
        let cql_query = format!(
            "DELETE {} FROM msgs.channel_follower WHERE {}",
            del_col, where_str
        );
        //let cql_query = "DELETE " + del_col + " FROM msgs.channel_follower WHERE " + where_str ;
...
pub fn get_channel_follower_by_channel_cid_and_profile_cid(
    session: impl FCQueryExecutor,
    p1: i64,
    p2: i64,
