...
impl Messages {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        ...
        let cql_query = format!(
            "INSERT INTO msgs.messages ({}) VALUES ({})",
...
impl Messages_Updater {
    ...
    pub fn update(&mut self, session: impl FCQueryExecutor) -> cdrs::error::Result<Frame> {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            ...
            format!(
                "UPDATE msgs.messages SET {} WHERE {}",
    ...
    pub fn update_data(&mut self, val: &Vec<u8>) -> &mut Self {
        self.updates
            .insert("data = ?", Blob::new(val.clone()).into());
        self
    }
...
pub fn get_messages_by_chat_id(
    session: impl FCQueryExecutor,
    p1: i64,
) -> Result<Messages, CWError> {
    let m = Messages_Selector::new().chat_id_eq(p1).get_row(session)?;
    Ok(m)
}
