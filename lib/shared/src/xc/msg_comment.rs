...
impl MsgComment {
    pub fn save(&self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        ...
        let cql_query = format!(
            "INSERT INTO msgs.msg_comment ({}) VALUES ({})",
...
impl MsgComment_Selector {
    ...
    pub fn _to_cql(&self) -> (String, Vec<Value>) {
        ...
        let mut cql_query = format!("SELECT {} FROM msgs.msg_comment", cql_select);
...
impl MsgComment_Updater {
    ...
    pub fn update(&mut self, session: impl FCQueryExecutor) -> cdrs::error::Result<Frame> {
        ...
        let mut cql_query = if self.wheres.is_empty() {
            format!("UPDATE msgs.msg_comment SET {}", cql_update)
        } else {
            format!(
                "UPDATE msgs.msg_comment SET {} WHERE {}",
...
impl MsgComment_Deleter {
    ...
    pub fn delete(&mut self, session: impl FCQueryExecutor) -> Result<(), CWError> {
        ...
        let cql_query = format!(
            "DELETE {} FROM msgs.msg_comment WHERE {}",
            del_col, where_str
        );
        //let cql_query = "DELETE " + del_col + " FROM msgs.msg_comment WHERE " + where_str ;
...
pub fn get_msg_comment_by_message_gid_and_comment_gid(
    session: impl FCQueryExecutor,
    p1: i64,
    p2: i64,
) -> Result<MsgComment, CWError> {
